...
# banco de dados tratado para sexo
  ...
  categorias <- levels(dados_negros$GrauInstrucao)
  
  # Criar séries para cada sexo
  serie_masculino <- dados_negros %>%
    filter(Sexo == "Masculino") %>%
    arrange(GrauInstrucao) %>%
    pull(Populacao)
  
  serie_feminino <- dados_negros %>%
...
output$plot_1 <- renderHighchart({  
  ...
  #replicar codigo acima para todos os grafico
...
output$plot_2 <- renderHighchart({
  ...
  highchart() %>%
    ...
    hc_xAxis(categories = levels(dados_nao_negros$GrauInstrucao)) %>%
    hc_yAxis(title = list(text = "População estimada"),
             labels = list(format = "{value:,.0f}")) %>%
    hc_add_series(name = "Masculino",
                  data = dados_nao_negros %>% filter(Sexo == "Masculino") %>% pull(Populacao),
                  color = "#1f77b4") %>%
    hc_add_series(name = "Feminino",
                  data = dados_nao_negros %>% filter(Sexo == "Feminino") %>% pull(Populacao),
                  color = "#ff7f0e")
})
}